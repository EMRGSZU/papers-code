...
import numpy as np
from pamodel import Attention_group, Learning_group, multimetrics,classification,plot_confusion_matrix,plot_feature
...
train_data_name = './data/pamap.npz'
...
for i in range(1):
    ...
    group_dif = torch.ones(27)
    group_dif = torch.chunk(group_dif, chunks=27, dim=0)
    ...
    para_list = []
    para_list.append({'params': AFS_atten.parameters(), 'initial_lr': 0.02})
    para_list.append({'params': AFS_cla.parameters(), 'initial_lr': 0.02})

    optimizer = torch.optim.SGD(para_list, lr=0.02, momentum=0.8, dampening=0.4, weight_decay=0.0001)
    # optimizer = torch.optim.Adam(para_list, lr=0.02, weight_decay=0.0001)
    ...
    output_file.write("pamap sensor selection{}".format(sen_weight) + '\n')
    acc_temp = []
    f1_temp = []
    auc_temp = []
    '''
    for K in range(5, 235, 10):
        # clf = DecisionTreeClassifier()
        # clf = SVC(C=0.4)
        clf = MLPClassifier()
        # clf = MLPClassifier(solver='lbfgs', activation='logistic')
        train_data = train_data.reshape(train_data.shape[0], 297)
        test_data = test_data.reshape(test_data.shape[0], 297)
        clf.fit(train_data[:, AFS_weight_rank[:K]], np.argmax(train_label, axis=1))
        pred_y = clf.predict(test_data[:, AFS_weight_rank[:K]])
        ...
        f1_temp.append(f1)
        auc_temp.append(auc)
        print("fused_net/pamap Using top {} features/ accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(K, acc, f1,
                                                                                                     auc))
    '''
    AFS_atten.eval()
    # AFS_learn.eval()
    ...
    pred_y = []
    true_y = []
    tes_gro_dif = torch.ones(27)
    tes_gro_dif = torch.chunk(tes_gro_dif, chunks=27, dim=0)
    ...
    f1_temp.append(f1)
    auc_temp.append(auc)
    print("fused_net/pamap/ accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(acc, f1, auc))
    
    cm = confusion_matrix(true_y, pred_y)
    # labels_name = ["A1","A2","A3","A4","A5","A6","A7","A8","A9","A10","A11","A12","A13"]
    labels_name = ["Lying","Sitting","Standing","Walking","Running",
    ...
    # plot_confusion_matrix(cm, labels_name, "PAMAP2")
    # plt.savefig("pamcm.pdf",bbox_inches="tight")
    # plt.show()
    plot_feature(atten_weight)
    # plt.savefig("pamfea.pdf",bbox_inches="tight")
    plt.show()
...
for i_idx in range(len(acc_avg)):
    print("Fused_net/pamap/ accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(acc_avg[i_idx], f1_avg[i_idx], auc_avg[i_idx]))
    output_file.write("Fused_net/pamap/{} accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(5+i_idx*10,acc_avg[i_idx], f1_avg[i_idx], auc_avg[i_idx]) + '\n')
output_file.write('\n')
output_file.close()
