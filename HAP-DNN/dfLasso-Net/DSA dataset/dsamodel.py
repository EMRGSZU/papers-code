...
s_num = 15 #sensor number
g_num = 45 #channel number
...
class Learning_group(nn.Module):
    def __init__(self, input_size=g_num*f_num, L_node=g_num*f_num//2, output_size=19):
...
def plot_feature(list_rank):
    ...
    y_name = ["T_xacc","T_yacc","T_zacc","T_xgyro","T_ygyro","T_zgyro","T_xmag","T_ymag","T_zmag",
    ...
    plt.colorbar()
    plt.xticks(x_local, x_name, fontsize=6, rotation=90)  # 将标签印在x轴坐标上
    plt.yticks(y_local, y_name, fontsize=6, rotation=0)  # 将标签印在y轴坐标上
    plt.title("DSA")