...
for i in range(10):
    ...
    optimizer = torch.optim.SGD(
        [{'params': AFS_atten.parameters(), 'initial_lr': 0.1}, {'params': AFS_learn.parameters(), 'initial_lr': 0.1}],
        lr=0.1, momentum=0.8, dampening=0.4, weight_decay=0.0001)
    ...
    for idx_epoch in range(start_epoch + 1, cs_epoch + 1):
        for idx, (inputs, labels) in enumerate(trainloader):
            inputs = inputs.reshape(int(inputs.shape[0]), 264)
    ...
    for idx_epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        for idx, (inputs, labels) in enumerate(trainloader):
            ...
            cs_temp = AFS_cs(inputs.reshape(int(inputs.shape[0]), 264))
            cs_temp = cs_temp.reshape(inputs.shape[0], 24, 11)
            ...
            loss.backward()
            optimizer.step()
            print('RRSS {}// Training batchsize {} epoch {}/{}  average loss: {}'.format(i + 1, batch_train, idx_epoch,
    ...
    f1_temp = []
    auc_temp = []
    for K in range(5, 170, 10):
        AFS_class = Classifier(input_size=K)
        AFS_class = AFS_class.to(device)
        optimizer1 = torch.optim.ASGD([{'params': AFS_class.parameters(), 'initial_lr': 0.1}], lr=0.1,
                                      weight_decay=0.0001)
        for temp_epoch in range(start_epoch + 1, end_epoch + 1):
            ...
            for data in trainloader:
                ...
                l_epoch = 0
                train_inputs = train_inputs.reshape(int(train_inputs.shape[0]), 264)
        ...
        for data in testloader:
            ...
            test_inputs = test_inputs.reshape(int(test_inputs.shape[0]), 264)
        ...
        f1_temp.append(f1)
        auc_temp.append(auc)
        print("HAP-DNN Using top {} features/ accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(K, acc, f1, auc))
        #output_file.write("AFS_group Using top {} features/accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(K, acc, f1, auc) + '\n')
...
for i_idx in range(len(acc_avg)):
    print("HAP-DNN Using top {} features/ accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(i_idx*10+5, acc_avg[i_idx], f1_avg[i_idx], auc_avg[i_idx]))
    output_file.write("HAP-DNN Using top {} features/ accuracy:{:.4f} /f1 score:{:.4f} /AUC:{:.4f}".format(i_idx*10+5, acc_avg[i_idx], f1_avg[i_idx], auc_avg[i_idx]) + '\n')
output_file.write('\n')
output_file.close()
