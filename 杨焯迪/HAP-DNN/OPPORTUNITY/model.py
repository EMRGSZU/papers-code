...
class CSmodule(nn.Module):
    def __init__(self, input_size=3003, E_node=300):  # 0.1 compress
        super(CSmodule, self).__init__()
        self.encoder = nn.Linear(input_size, E_node)
        self.decoder = nn.Sequential(
            nn.Linear(E_node, 1001, bias=True),
            nn.Tanh(),
            nn.Linear(1001, input_size, bias=True),
            nn.Tanh(),
...
class Attention_group(nn.Module):
    def __init__(self):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(143, 71, bias=True),
            nn.Tanh(),
            nn.Linear(71, 143, bias=True),
            nn.Tanh()
        )
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, x):
        ...
        for j in range(x.shape[1]):
            if j in group_weight_rank[:143//2]:
                ...
                A[:, j, :] = atten_temp
        x = x.contiguous().view(int(x.shape[0]), 3003)
        A = A.contiguous().view(int(A.shape[0]), 3003)
        G = torch.mul(x, A)
        return G, A


class Learning_group(nn.Module):
    def __init__(self, input_size=3003, L_node=1001, output_size=4):
...
class Classifier(nn.Module):
    def __init__(self, input_size, L_node=1001, output_size=4):
...
def multimetrics(y_true, y_pred):
    ...
    auc = []
    for i in range(4):
        f1.append(f1_score(y_true == i, y_pred == i, labels = True))
        acc.append(accuracy_score(y_true == i, y_pred == i))
        auc.append(roc_auc_score(y_true == i, y_pred == i))
    return f1, acc, auc
