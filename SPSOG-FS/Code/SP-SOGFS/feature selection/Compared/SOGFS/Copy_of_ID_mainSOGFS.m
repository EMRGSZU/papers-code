...
%     resultMat7 = zeros(length(gamma)*length(d)*(k-1),feaLen+3);
    idMat5 = zeros(length(gamma)*length(d)*(k-1),dim+3);
    for iIndex = 1:length(gamma)
        ...
        for dtIndex = 1:length(d)
            ...
            for j =2:k
                ...
                idMat5(matIndex,1:end-3) = id';
                idMat5(matIndex,end-2:end) =  [i,dt,j];
...
toc
save("C:\Users\Administrator\Desktop\AMDV16_ID_JAFFE5.mat",'idMat5','k','gamma','d')
