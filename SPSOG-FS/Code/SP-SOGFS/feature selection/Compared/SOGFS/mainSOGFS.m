...
if isToPara == 0
    ...
    resultMat = zeros(length(gamma)*length(d)*(k-1),feaLen+3);
    for iIndex = 1:length(gamma)
        ...
        for dtIndex = 1:length(d)
            ...
            for j =2:k
                ...
                resultMat(matIndex,1:feaLen) = para(id,FeaNumCandi,fea,gnd,nClass);
                resultMat(matIndex,feaLen+1:feaLen+3) = [i,dt,j];
            end
        end
    end
...
% save("F:\Users\cnnyl\Desktop\SP-SOGFS\feature selection\Result\SP\BA.mat",'bestAcc','accInd','resultMat')
