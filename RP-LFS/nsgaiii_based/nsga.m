function [final_member, best_member, hv_arr, igd_arr] = nsga(X, Y, Para)
    % 参数设置
...
%     n_crossover = floor(n_pop / 2);
    ...
    hv_arr = zeros(n_iter, 1);
    igd_arr = zeros(n_iter, 1);
    ...
    normal_best_member = repmat(individual, n_iter, n_number);
...
%     stop_cnt = 1;
    for iter = 1:n_iter
        ...
        for num = 1:n_number
            ...
            pop(num,:) = pop_temp.';
            F1 = pop_temp(F{1});
            hv_sum = hv_sum + HV(F1, optimun);
            igd_sum = igd_sum + IGD(F1, optimun);
            [best_member(iter,num),~] = analyse(F1 , F{1}); %因为要替换掉原种群中best的那个位置，所以加入best_pop代表最好的种群
            normal_best_member(iter,num) = best_member(iter,num);
            %if num == n_number
        ...
        iter_time2 = clock;
        hv_arr(iter,1) = hv_sum/n_number;
        igd_arr(iter, 1) = igd_sum/n_number;
    ...
    final_member = best_member(n_iter,:);
end
...
function [best_member,best_pop] = analyse(F1, not_dominate)
    ...
    for i = 2:n_F1
        ...
        elseif F1(i).cost(1) == best_member.cost(1) && size(F1(i).crossPoint,2) > size(best_member.crossPoint,2)
            best_member = F1(i);
            best_pop = not_dominate(i);
        elseif F1(i).cost(1) == best_member.cost(1) && size(F1(i).crossPoint,2) == size(best_member.crossPoint,2) && F1(i).cost(2) < best_member.cost(2)
            best_member = F1(i);
            best_pop = not_dominate(i);
        end
    end
