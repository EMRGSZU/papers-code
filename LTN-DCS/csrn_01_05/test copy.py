        ...
        with torch.no_grad():
            for i in range(len(filelist)):
                if os.path.splitext(filelist[i])[1] in ['.tif', '.bmp', '.png', '.jpg']:
                    ...
                    img_y_o = process_img(img_ori, only_y=True)
                    img_y, hei, wid, hei_pad, wid_pad  = image_padding(img_y_o)
                    img = img_y / 255.0
                    ...
                    for prediction in predictions_list:
                        temp_psnr = calc_psnr(np.array(np.round(prediction[0][0]), dtype = 'uint8'), img_y_o)
                        temp_ssim = calc_ssim(np.array(np.round(prediction[0][0]), dtype = 'uint8'), img_y_o)
