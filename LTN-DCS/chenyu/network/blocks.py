...
class DeepNet(nn.Module):
    ...
    def forward(self, init_img, fmaps):
        ...
        # # res_img为残差图像
        # res_img = res_img.cpu().data.numpy()
        # res_img = np.clip(res_img, 0, 1)
        # res_img *= 255
        # img_ = Image.fromarray(np.array(np.round(res_img[0][0]), dtype='uint8'))
        # img_.save('testResidual.png')
        # init_img = init_img.cpu().data.numpy()
        # init_img = np.clip(init_img, 0, 1)
        # init_img *= 255
        # img_init = Image.fromarray(np.array(np.round(init_img[0][0]), dtype='uint8'))
        # img_init.save('testInit.png')
