...
%                      para(id,FeaNumCandi,fea,gnd,nClass), 0,k];
    ...
    idMat1 = zeros(length(gamma)*(k-1),dim+2);
%     resultMat = zeros(length(gamma)*(k-1),feaLen+2);
    for iIndex = 1:length(gamma)      
        ...
        for j =2:k
            ...
            idMat1(matIndex,1:end-2) = id';
            idMat1(matIndex,end-1:end) =  [i,j];
...
save("C:\Users\cnnyl\Desktop\AMDV16_ID_SRBCT1.mat",'idMat1','k','gamma')
