...
%                      para(id,FeaNumCandi,fea,gnd,nClass), 0,k];
    ...
    resultMat6 = zeros(length(gamma)*(k-1),feaLen+2);
%     idMat1 = zeros(length(gamma)*length(d)*(k-1),dim+3);
    for iIndex = 1:length(gamma)
        ...
        for j =2:k            
            ...
            id = idMat6(matIndex,1:end-2)';
            resultMat6(matIndex,1:feaLen) = para(id,FeaNumCandi,fea,gnd,nClass);
            resultMat6(matIndex,feaLen+1:feaLen+2) = [i,j];
        end
    end
    [bestAcc,accInd] = max(resultMat6); 
    bestAcc = bestAcc(1:end-2);
    accInd = accInd(1:end-2);
end
toc
% save("C:\Users\Administrator\Desktop\AMDV16_ORL1.mat",'idMat1','k','gamma','d')
save("F:\Users\cnnyl\Desktop\AMDV16_BA6.mat",'bestAcc','accInd','resultMat6','idMat6','k','gamma')
