...
%     resultMat7 = zeros(length(gamma)*length(d)*(k-1),feaLen+3);
    idMat4 = zeros(length(gamma)*length(d)*(k-1),dim+3);
    for iIndex = 1:length(gamma)
        ...
        for dtIndex = 1:length(d)
            ...
            for j =2:k
                ...
                idMat4(matIndex,1:end-3) = id';
                idMat4(matIndex,end-2:end) =  [i,dt,j];
...
toc
save("C:\Users\cnnyl\Desktop\AMDV16_ID_ecoli4.mat",'idMat4','k','gamma','d')
