...
%                      para(id,FeaNumCandi,fea,gnd,nClass), 0,k];
    ...
    resultMat1 = zeros(length(gamma)*length(d)*(k-1),feaLen+3);
%     idMat1 = zeros(length(gamma)*length(d)*(k-1),dim+3);
...
%                 idMat1(matIndex,1:end-3) = id';
%                 idMat1(matIndex,end-2:end) =  [i,dt,j];
                id = idMat1(matIndex,1:end-3)';
                resultMat1(matIndex,1:feaLen) = para(id,FeaNumCandi,fea,gnd,nClass);
                resultMat1(matIndex,feaLen+1:feaLen+3) = [i,dt,j];
            end
        end
    end
    [bestAcc,accInd] = max(resultMat1); 
    bestAcc = bestAcc(1:end-3);
    accInd = accInd(1:end-3);
end
toc
% save("C:\Users\Administrator\Desktop\AMDV16_ORL1.mat",'idMat1','k','gamma','d')
save("F:\Users\cnnyl\Desktop\SOGFS_Result\BA\5\AMDV16_BA1.mat",'bestAcc','accInd','resultMat1','idMat1','k','gamma','d')
